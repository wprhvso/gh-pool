...
pool chrome [профиль]                              живой десктоп Chrome в браузере
...
`pool chrome` заводит браузерную сессию, печатает ссылку на её живой десктоп и
открывает её новым окном браузера. За ссылкой — KasmVNC с X-сервера раннера, тот
же экран, что пишет ffmpeg: настоящая мышь, настоящая клавиатура, буфер обмена в
обе стороны.

```bash
pool chrome                чистый профиль, сессия одноразовая
pool chrome shopping       на профиле shopping: куки и логины переживут сессию
pool chrome -s <id>        десктоп уже живущей сессии
pool chrome -p             плеер: запись и десктоп на одной странице
pool chrome -n             только напечатать ссылку, ничего не открывать
```

Сессия уезжает в очередь пула, поэтому раннер подхватывает её не мгновенно:
команда ждёт `-w` секунд (по умолчанию 600), пока десктоп не поднимется, и только
потом открывает браузер. `-w 0` открывает сразу, не дожидаясь.

Ссылки закрыты Basic-аутентификацией: пользователь `admin`, пароль —
`GH_POOL_TOKEN`. Он же нужен самой команде: браузерные сессии ходят под своим
токеном, а не под `GH_POOL_CLIENT_TOKEN`. Что там внутри — в
...
Клиент: `GH_POOL_SERVER`, `GH_POOL_CLIENT_TOKEN`, `GH_POOL_TOKEN` (браузерные сессии и `pool chrome`).
